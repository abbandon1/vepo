...
async function loadExcursions() {
    try {
        const paginatedUrl = `${API_URL}?page=${currentPage}&size=${pageSize}&sort=${currentSort}`;
        ...
        if (!Array.isArray(excursions)) {
            console.error('Ожидался массив, но пришло:', excursions);
            return;
        }

        if (excursions.length === 0 && currentPage > 0) {
            currentPage--;
            loadExcursions();
            return;
        }
        ...
        if (nextBtn) nextBtn.disabled = excursions.length < pageSize;
        if (prevBtn) prevBtn.disabled = currentPage === 0;
        if (pageInfo) pageInfo.innerText = `Страница ${currentPage + 1}`;

        excursions.forEach(item => {
            const name = item.name || 'Без названия';
            const price = typeof item.price === 'number' ? item.price : 0;
            const guideName = item.guideName || 'Не указан';
            const excursionType = item.excursionType || 'Не определен';
            
            tableBody.innerHTML += `
                        <tr class="border-b hover:bg-gray-50">
                            <td class="p-3 border-r border-gray-100 text-center">${item.id}</td>
                            <td class="p-3 border-r border-gray-100 font-medium text-gray-900">${name}</td>
                            <td class="p-3 border-r border-gray-100 text-center">${price.toFixed(2)} руб.</td>
                            <td class="p-3 border-r border-gray-100 text-xs text-gray-500 text-center">${item.from || ''} — ${item.to || ''}</td>
                            <td class="p-3 border-r border-gray-100">${guideName}</td>
                            <td class="p-3 border-r border-gray-100 text-center">
                                <span class="px-2 py-1 rounded-full text-xs font-medium ${excursionType === 'Групповая' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}">
                                    ${excursionType}
                                </span>
                            </td>
...
window.editExcursion = editExcursion;
